...
class LilLexiDoc extends Compositor {
	 ...
	 * setUI
	 */
	public void setUI(LilLexiUI ui) {this.ui = ui; composition.setUI(ui);}
	...
	public void add(String c) {
		...
		glyphs.add(cursor, new Character(c, cursor * 25, cursor * 40));
		cursor++;
	...
	public void add(int rect_size) {
		glyphs.add(cursor, new RectangleGlyph(20, cursor * 25, cursor * 40));
		cursor++;
		//glyphs.add(cursor, new Cursor(cursor, cursor, cursor));
		comp = composition.compose(glyphs);
		ui.updateUI();
